...
## safeArray.h
  - Array클래스를 상속받아 SafeArray클래스를 생성합니다
  - is a  : safeArray is Array
### class safeArray 정의
```c
tamplate <typename T>
...
## public 맴버변수 정의
  - 아래와 같이 전역변수, 생성자, 소멸자, 연산자를 정의합니다.
### 생성자 정의
  - Array 클래스의 protected 맴버인 getArraySize를 자식 클래스인 SafeArray쪽에서 접근 합니다.
  - explicit 키워드로 묵시적 형변환을 방지합니다.
```c
...
### 대괄호 연산자 템플릿 구현
  - 임의의 반환값을 T 타입으로 정의해 줍니다.
  - Array 클래스의 protected 맴버로 접근해 size_ 를 사용할 수 있다.
```c
template <typename T>
...
```
### array 생성
  - 내부적으로 int형을 저장하는 nums SafeArray 생성
  - <> 안에 타입을 받아 타입에 타겟팅 되는 SafeArray arr1 배열 초기화
